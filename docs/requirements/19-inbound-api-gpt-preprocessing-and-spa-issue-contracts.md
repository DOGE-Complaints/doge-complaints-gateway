...
- **Introspection как источник авторства (GW-GAUTH-04):** когда gateway выполняет успешный introspection пользовательского токена у identity (см. EPIC-M2-20), **авторитетный идентификатор автора** для персистентного слоя — проверенный `sub` из ответа introspection, а не заявленный в теле `submitter.external_user_id`. Поле `submitter` в payload остаётся обязательным для контракта GPT Action, но при расхождении с `sub` побеждает introspection; отдельная параллельная модель авторства не вводится.
