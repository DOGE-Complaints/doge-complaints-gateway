...
- Обобщить `evaluate_verification_gate`/автор-резолвер под нейтральный `IntrospectionResult` (сейчас импорт из `introspection_client`) — согласовать с рефактором в [GW-DRAFT-04](./STORY-GW-DRAFT-04-safe-removal-orphaned-gpt-authz.md).
